...
class TurtleControlNode(Node):
    ...
    def control_loop(self):
        ...
        msg = Twist()
        kp_vel = 2.0 # P gain for velocity
        kp_ang = 6.0 # P gain for rotation

        # Error
        errorX = self.turtle_to_catch.x - self.pose.x
        errorY = self.turtle_to_catch.y - self.pose.y
        distance = math.sqrt(errorX * errorX + errorY * errorY)

        if distance > 0.5:

            msg.linear.x = distance * kp_vel # assing velocity * P gain

            # Rotate to target
            goalTheta = math.atan2(errorY, errorX)
            diff = goalTheta - self.pose.theta
            if diff > math.pi:
                diff -= 2*math.pi
            elif diff < -math.pi:
                diff += 2*math.pi

            msg.angular.z = diff * kp_ang # assing angle * P gain
